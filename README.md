...
- 📈 **订阅额度显示（2026-09-16 新增）** —— 账号管理里一眼看到每个账号还剩多少额度：5 小时 / 本周 / 本月三条滚动窗口的用量进度条与百分比（接近上限变色提醒），并标注重置时间、套餐名与当期到期 / 是否续费；多个账号各查各的，独立成行、互不干扰。额度数据取自 Cline 账单接口，只读查询，**不消耗额度、不计费**
...
| 卡片 | 功能 |
|---|---|
| 账号管理 | 账号池增删改、显隐密钥、逐账号连通性测试、单账号/轮询模式、用量统计；每账号的额度进度（5 小时 / 本周 / 本月，含重置时间与上限，可手动刷新） |
